...
class WatermarkRemover(object):
    ...
    def __init__(self, threshold: int, kernel_size: int):
        ...
        # 截取手动划定水印范围那张图片所选用的帧的编号
        self.select_snapshot = 1000
    ...
    def select_roi(self, img: numpy.ndarray, hint: str) -> list:
        ...
        :return: 框选区域坐标
        """
        w, h = int(self.coff * img.shape[1]), int(self.coff * img.shape[0])
    ...
    @staticmethod
    def generate_single_mask(img: numpy.ndarray, roi: list, threshold: int) -> numpy.ndarray:
    ...
    def generate_watermark_mask(self, video_path: str) -> numpy.ndarray:
        ...
        # amount_of_frames = video.get(cv2.CAP_PROP_FPS)
        video.set(cv2.CAP_PROP_POS_FRAMES, self.select_snapshot)
    ...
    def generate_subtitle_mask(self, frame: numpy.ndarray, roi: list) -> numpy.ndarray:
        ...
        # 仅使用ROI横坐标区域
        mask = self.generate_single_mask(frame, [0, roi[1], frame.shape[1], roi[3]], self.threshold)
        return self.dilate_mask(mask)

    @staticmethod
    def repair_paint_image(img: numpy.ndarray, mask: numpy.ndarray) -> numpy.ndarray:
        ...
        :return: 修复后图像
        """
        re = cv2.inpaint(img, mask, 1, cv2.INPAINT_TELEA)
        return re

    @staticmethod
    def merge_audio(input_path: str, output_path: str, temp_path: str):
    ...
    def remove_video_watermark(self):
        ...
        for i, name in enumerate(filenames):
            ...
            while success:
                frame = self.repair_paint_image(frame, mask)
...
if __name__ == '__main__':
    # 去除视频水印
    remover = WatermarkRemover(threshold=83, kernel_size=3)
    # remover = WatermarkRemover(threshold=235, kernel_size=30)
    remover.remove_video_watermark()

    # 去除视频字幕
    # remover = WatermarkRemover(threshold=235, kernel_size=4)
